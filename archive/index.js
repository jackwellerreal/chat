    ...
    var replaced = string
      ...
      .replace(
        ...
        `<a class="message-highlight" href="https://www.google.com/finance/quote/${string
          .match(/\$([^ ^\!^\@^\#^\^]+)/g)
          ?.map((match) => match.replace("$", ""))}" target="_blank">$&</a>`
      )
      .replace(
        /(?<![:/#])\^([^ ^\!^\@^\#^\$]+)/g,
...
const channel =
  urlParams.get("text-channel") == null
    ...
    : channels.find(
        (obj) =>
          obj.name === urlParams.get("text-channel") && obj.type === "text"
      )
...
async function displayPosts(posts) {
  try {
    ...
    posts.forEach((post, i) => {
      ...
      const mention = message.includes(`@${localStorage.getItem("name")}`)
        ? "message-mention"
        : "";
      ...
      if (previousMessage) {
        const previousMessageName =
          previousMessage.children[0].children[0].innerHTML.split(" ")[0];
        ...
        if (previousPreviousMessage) {
          if (previousPreviousMessage.classList.contains("message-multi-end")) {
            const previousPreviousMessageName =
              previousPreviousMessage.getAttribute("og-name");

            if (previousMessageName == previousPreviousMessageName) {
              previousMessage.classList.add("message-multi-start");
            }
          }
        }
      }
...
function loadHome() {
  ...
  element.innerHTML = `
      <span class="message-content">ok</span>
    `;
...
if (server.version == clientVersion) {
  ...
  if(urlParams.get("page") == "messages") {
    if (urlParams.get("server-id")) {
      if (urlParams.get("text-channel")) {
        loadData();
      }
        ...
        getDownloadURL(storageRef)
          .then(async (url) => {
            await addDoc(messageRef, {
              ...
              bot: false,
              owner: localStorage.getItem("owner") == "true" ? true : false,
              content: `image:${url}`,
              colour: color,
              timestamp: new Date(),
              ip: ip,
    ...
    if (message.startsWith("/")) {
      if (channel == "bot-spam") {
        if (message.startsWith("/verify")) {
          if (server.codes.includes(message.replace("/verify ", ""))) {
        ...
        if (message.startsWith("/random")) {
          await addDoc(messageRef, {
            name: name,
            verified: localStorage.getItem("verified") == "true" ? true : false,
            bot: true,
            command: message,
        ...
        if (message.startsWith("/slots")) {
          if (message.replace("/slots ", "") != "/slots") {
            ...
            if (slots1 == slots2 && slots2 == slots3 && slots3 == slots1) {
              ...
              status =
                "u lost " + message.replace("/slots ", "") + " moneys lol";
            }
            await addDoc(messageRef, {
              name: name,
              verified:
